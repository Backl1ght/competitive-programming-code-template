...
struct SGTree {
    ...
    {
        int lc = l[x], rc = r[x];
        if (alpha * s[x] <= max(s[lc], s[rc])) return false;
        if (alpha * s[x] >= sd[x]) return false;
        return true;
    }
    ...
    {
        if (!rt) return 0;
        if (v[rt] == val && cnt[rt]) return sz[l[rt]];
        if (v[rt] < val) return sz[l[rt]] + cnt[rt] + getPrevRank(r[rt], val);
    ...
    {
        if (!rt) return 1;
        if (v[rt] == val && cnt[rt]) return sz[l[rt]] + cnt[rt] + 1;
        if (v[rt] < val) return sz[l[rt]] + cnt[rt] + getSuccRank(r[rt], val);
    ...
    {
        if (!rt) return 0;
        if (k <= sz[l[rt]]) return getKth(l[rt], k);
        if (k - sz[l[rt]] <= cnt[rt]) return v[rt];
