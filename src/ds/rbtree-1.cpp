...
template <typename T>
void rbtree<T>::connect34(Node* nroot, Node* nlc, Node* nrc, Node* ntree1, Node* ntree2, Node* ntree3, Node* ntree4) {
  nlc->lc = ntree1;
  if (ntree1 != NULL)
    ntree1->ftr = nlc;
  nlc->rc = ntree2;
...
template <typename T>
void rbtree<T>::previs(Node* ptn, int cnt) {
  if (ptn == NULL) {
    if (blackheight == -1)
      blackheight = cnt;
...
template <typename T>
void rbtree<T>::invis(Node* ptn, int cnt) {
  if (ptn == NULL) {
    if (blackheight == -1)
      blackheight = cnt;
    assert(blackheight == cnt);
...
template <typename T>
void rbtree<T>::postvis(Node* ptn, int cnt) {
  if (ptn == NULL) {
    if (blackheight == -1)
      blackheight = cnt;
    assert(blackheight == cnt);
...
template <typename T>
void rbtree<T>::checkconnect(Node* ptn) {
  if (!ptn)
    return;
  assert(ptn->s > 0);
...
template <typename T>
void rbtree<T>::correctlyconnected() {
  checkconnect(_root);
}
...
template <typename T>
typename rbtree<T>::Node* rbtree<T>::findkth(int rank, Node* ptn) {
  if (!(ptn->lc)) {
    if (rank == 1) {
      return ptn;
    } else {
...
template <typename T>
int rbtree<T>::find_rank(T v, Node* ptn) {
  if (!ptn)
    return 1;
  else if (ptn->val >= v)
...
template <typename T>
typename rbtree<T>::Node* rbtree<T>::find(T v, const int op) {
  Node* ptn = _root;
  _hot = NULL;
  while (ptn != NULL) {
    _hot = ptn;
...
template <typename T>
typename rbtree<T>::Node* rbtree<T>::rfind(T v, const int op) {
  Node* ptn = _root;
  _hot = NULL;
...
template <typename T>
typename rbtree<T>::iterator rbtree<T>::insert(T v) {
  Node* ptn = find(v, 1);
  if (_hot == NULL) {
    init(v);
...
template <typename T>
typename rbtree<T>::iterator rbtree<T>::lower_bound(T v) {
  Node* ptn = _root;
  while (ptn) {
    _hot = ptn;
    if (ptn->val < v) {
...
template <typename T>
typename rbtree<T>::iterator rbtree<T>::upper_bound(T v) {
  Node* ptn = _root;
  while (ptn) {
    _hot = ptn;
    if (ptn->val > v) {
