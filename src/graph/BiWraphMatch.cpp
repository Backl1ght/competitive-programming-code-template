...
struct biwraph {
    ...
    void bfs(int now) {
        ...
        while(true) {
            ...
            T a = TMAX;
            for (int i = 1; i <= n; ++i) if (!visr[i]) a = min(a, slack[i]);
            for (int i = 1; i <= n; ++i) {
                if (visl[i]) highl[i] -= a;
                if (visr[i]) highr[i] += a;
                else slack[i] -= a;
            ...
            for (int i = 1; i <= n; ++i) 
                if (!visr[i] && !slack[i] && check(i)) return;
        }
    }
    ...
    void match() {
        ...
        for (int i = 1; i <= n; ++i) highl[i] = *max_element(G[i].begin() + 1, G[i].end());
    ...
    T getMaxMatch() {
        ...
        match();
        for (int i = 1; i <= n; ++i) {
            if (G[i][matchl[i]] > 0) res += G[i][matchl[i]];
            else matchl[i] = 0;
        }
        return res;
    }
