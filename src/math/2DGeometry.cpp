namespace Geometry
{
    ...
    struct line
    {
        ...
        // 点和倾斜角确定直线
        line(const point& a, const double b) : s(a) {
            if (sgn(b - pi / 2) == 0) e = s + point(0, 1);
            else e = s + point(1, tan(b));
        }
        ...
        int isSegCrossSeg(line l) {
            ...
            return (d1 == 0 && sgn(dot(l.s - s, l.s - e)) <= 0) 
                || (d2 == 0 && sgn(dot(l.e - s, l.e - e)) <= 0)
                || (d3 == 0 && sgn(dot(s - l.s, s - l.e)) <= 0)
                || (d4 == 0 && sgn(dot(e - l.s, e - l.e)) <= 0);
        }
    ...
    struct circle
    {
        ...
        int relationToPoint(point a) {
            ...
            if(sgn(d2 - r * r) < 0) return 2;
            else if(sgn(d2 - r * r) == 0) return 1;
            return 0;
        }
    ...
    struct polygon
    {
        ...
        double perimeter() {
            double sum = 0;
            for(int i = 0; i < n; i++) sum += (p[(i + 1) % n] - p[i]).length();
            return sum;
        }
        ...
        struct cmp {
            ...
            bool operator () (const point& a,const point& b) const {
                int d = sgn(det(p, a, b));
                if(d == 0) return sgn(distance(a, p) - distance(b, p)) < 0;
                return d > 0;
            }
        };
        ...
        polygon getComvex() {
            ...
            int top = 2;
            for(int i = 2; i < n; i++) {
                while(top > 1 && sgn(det(convex.p[top - 2], convex.p[top - 1], p[i])) <= 0) --top;
                convex.p[top++] = p[i];
            }
            convex.resize(top);
            if(convex.n == 2 && convex.p[0] == convex.p[1]) convex.resize(1);
        ...
        pair<point, point> getMaxPair() {
            ...
            for (int i = 0; i < n; ++i) {
                int j = (i + 1) % n;
                while(sgn(det(p[i], p[j], p[k]) - det(p[i], p[j], p[(k + 1) % n])) <= 0) k = (k + 1) % n;

                if (sgn(distance(p[i], p[k]) - dis) > 0) p1 = p[i], p2 = p[k], dis = distance(p1, p2);
                if (sgn(distance(p[j], p[k]) - dis) > 0) p1 = p[j], p2 = p[k], dis = distance(p1, p2);
            }
        ...
        circle getMinCircle() {
            ...
            circle c(p[0], 0);
            for (int i = 0; i < n; ++i) {
                if (c.relationToPoint(p[i]) == 2) continue;
                c.p = (p[0] + p[i]) / 2; 
        ...
        // 3 点上
        int relationToPoint(point a) {
            for (int i = 0; i < n; ++i) if (p[i] == a) return 3;

            getline();
            for (int i = 0; i < n; ++i) if (l[i].relationToPoint(a) == 3) return 2;
