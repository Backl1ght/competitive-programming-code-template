namespace Geometry3 {
    ...
    struct point3 {
        ...
        bool operator < (const point3& p) const {
            ...
            if (sgn(y - p.y) != 0) return sgn(y - p.y) < 0;
            return sgn(z - p.z) < 0;
        }
    ...
    struct line3 {
        ...
        line3(point3 _s = point3(), point3 _e = point3()) : s(_s), e(_e) {}
        ...
        point3 rotate(point3 p, double alpha) {
            if (sgn(det(p, s, e).length()) == 0) return p;
            point3 p1 = det(s, e, p);
            point3 p2 = det(e - s, p1);
            double len = det(p, s, e).length() / distance(s, e);
            p1 = p1.trunc(len); p2 = p2.trunc(len);
            point3 p3 = p + p2;
            point3 p4 = p3 + p1;
    ...
    struct plane {
        ...
        plane(point3 _a, point3 _o) : a(_a), o(_o) {}
    ...
    struct polygon3 {
        ...
        polygon3() : n(0) {}
        polygon3(int _n) : n(_n), P(n), F(8 * n), G(n, vector<int>(n)) {}
        ...
        void buildConvex3() {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (cmp(P[i], add) > 0) swap(add.b, add.c);
        ...
        // 三维凸包表面积(POJ3528)
        double area() {
            if (n == 3) return det(P[0], P[1], P[2]).length() / 2;
