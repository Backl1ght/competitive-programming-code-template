...
struct Simplex {
    ...
    void input() {
        for (int i = 1; i <= n; ++i) read(A[0][i]); // A_{0,i} = c_i
        for (int i = 1; i <= m; ++i) {
    ...
    void init() {
        flag = OK;
        idx[0] = INT_MAX; for (int i = 1; i <= m; ++i) idx[i] = n + i;
        idy[0] = INT_MAX; for (int i = 1; i <= n; ++i) idy[i] = i;

        for(;;) {
            int x = 0, y = 0;
            for (int i = 1; i <= m; ++i) if (b[i] < -eps && idx[i] < idx[x]) x = i;
            if (!x) break;
            
            for (int i = 1; i <= n; ++i) if (A[x][i] < -eps && idy[i] < idy[y]) y = i;
            if (!y) { flag = INFEASIBLE; break; }
    ...
    void simplex() {
        for(;;) {
            int x = 0, y = 0;
            for (int i = 1; i <= n; ++i) if (A[0][i] > eps && idy[i] < idy[y]) y = i;
            if (!y) break;

            for (int i = 1; i <= m; ++i) if (A[i][y] > eps) {
                if (!x) x = i;
                else {
                    double delta = b[i] / A[i][y] - b[x] / A[x][y];
                    if (delta < -eps) x = i;
                    else if (delta < eps && idx[i] < idx[x]) x = i; 
                }
            }
            if (!x) { flag = UNBOUNDED; break; }
    ...
    void work() {
        ...
        if (flag == OK) {
            for (int i = 1; i <= n; ++i) {
                x[i] = 0;
                for (int j = 1; j <= m; ++j) if (idx[j] == i) { x[i] = b[j]; break; }
            }
        }
    }
